...
SCRIPT="$REPO_ROOT/abc/abc-export"
...
compare_text_file() {
	...
	local actual="$2"
	local label="$3"
	assert_eq "$(cat "$expected")" "$(cat "$actual")" "$label"
}
...
run_case() {
	(
		...
		unset ABC_EXPORT_CALL EXPECT_EXIT_CODE EXPECT_EXPORT_DIR
		ABC_EXPORT_CALL=()
		EXPECT_EXIT_CODE=0
		...
		if [[ ${#ABC_EXPORT_CALL[@]} -eq 0 ]]; then
			fail "missing ABC_EXPORT_CALL in $case_name/case.conf"
		fi

		copy_repo_fixture "$case_dir/repo" "$repo_dir"

		set +e
		"$SCRIPT" "${ABC_EXPORT_CALL[@]}" >"$stdout_file" 2>"$stderr_file"
		status=$?
		set -e

		assert_eq "$EXPECT_EXIT_CODE" "$status" "$case_name exit code"

		if [[ -f "$expected_dir/stdout.txt" ]]; then
			compare_text_file "$expected_dir/stdout.txt" "$stdout_file" "$case_name stdout"
		fi
		if [[ -f "$expected_dir/stdout.contains.txt" ]]; then
			check_contains_file "$expected_dir/stdout.contains.txt" "$stdout_file"
		fi
		if [[ -f "$expected_dir/stderr.txt" ]]; then
			compare_text_file "$expected_dir/stderr.txt" "$stderr_file" "$case_name stderr"
		fi
		...
		case "$EXPECT_EXPORT_DIR" in
			absent)
				assert_not_exists "$out_dir"
				;;
			empty)
				assert_dir_exists "$out_dir"
				assert_eq "" "$(list_export_files "$out_dir")" "$case_name export tree"
				;;
			present)
				assert_dir_exists "$out_dir"
				;;
		...
		if [[ -d "$out_dir" && -f "$expected_dir/readme.contains.txt" ]]; then
			assert_file_exists "$out_dir/README.md"
			check_contains_file "$expected_dir/readme.contains.txt" "$out_dir/README.md"
		fi

		if [[ -f "$case_dir/verify.sh" ]]; then
			CASE_DIR="$case_dir" REPO="$repo_dir" OUT_DIR="$out_dir" WORK_DIR="$work_dir" \
...
main() {
	...
	for case_dir in "$TEST_ROOT"/abc-export-*; do
		[[ -d "$case_dir" ]] || continue
		run_test "$case_dir"
	done
